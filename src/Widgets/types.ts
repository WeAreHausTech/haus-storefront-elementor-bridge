...
export type FeatureFunctionMappings = ExtractMethods<
  PluginFeatureMappings & UserFeatureMappings
>;
...
type RemoveFunctionsFromUnion<T> = T extends (...args: never) => unknown
  ? never
  : T;
...
type ExtractReturnType<K extends keyof FeatureFunctionMappings> = ReturnType<
  FeatureFunctionMappings[K]
> extends boolean
  ? boolean
...
export interface BaseDataProviderProps {
  ...
  options?: Maybe<Record<string, unknown>> & {
    enabledFeatures?: Maybe<Features>;
  } & {
